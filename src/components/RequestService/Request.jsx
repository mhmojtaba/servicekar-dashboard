...
const AddRequest = () => {
	...
	useEffect(() => {
		if (isClient && user) {
			...
			if (user?.location?.lat && user?.location?.lng) {
				setLocation([user.location.lat, user.location.lng]);
			}
		}
	}, [user, isClient]);
	...
	useEffect(() => {
		if (isClient && selectedAddress !== null) {
			setRequestData({
				...requestData,
			...
			// Reset to user's profile data when no selected address
			setRequestData((prev) => ({
				...prev,
				first_name: user?.first_name || "",
				last_name: user?.last_name || "",
			...
			// Reset location to user's location if available
			if (user?.location?.lat && user?.location?.lng) {
				setLocation([user.location.lat, user.location.lng]);
			} else {
				setLocation([32.644397, 51.667455]); // Default location
			}
		}
	}, [selectedAddress, user, isClient]);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (res?.msg === 0) {
				// Reset form but keep user's profile data as defaults
				setRequestData({
					id_service: null,
					requester_type: 1,
					operation_type: null,
				...
				// Reset location to user's location or default
				if (user?.location?.lat && user?.location?.lng) {
					setLocation([user.location.lat, user.location.lng]);
				} else {
											...
											<div className="space-y-3">
												...
												<Select
													...
													value={operation_type.find(
														(option) =>
															option.value == requestData.operation_type
													)}
													onChange={(e) =>
														setRequestData({
															...requestData,
